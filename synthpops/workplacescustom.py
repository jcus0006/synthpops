...
class WorkplaceCustom(spb.LayerGroup):
    """
...
def add_workplace(pop, workplace):
    ...
    """
    if not isinstance(workplace, WorkplaceCustom):
...
def initialize_empty_workplaces(pop, n_workplaces=None):
    ...
    pop.workplaces = [WorkplaceCustom() for nw in range(pop.n_workplaces)]
    return
...
def get_uids_workers(empstatus_by_uid, age_by_uid):
    """
    ...
    potential_worker_uids_temp = deepcopy(empstatus_by_uid)
    ...
    for uid in empstatus_by_uid:
        if empstatus_by_uid[uid] != 0:
            potential_worker_uids_temp.pop(uid, None)

    for uid in potential_worker_uids_temp:
        ai = age_by_uid[uid]

        potential_worker_uids[uid] = ai
